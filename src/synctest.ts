...
async function syncSelfFollowers(ctx: AppContext) {
    ...
    const followsToDelete = currentFollowers.filter(x => !actualFollowers.map(b => b.uri).includes(x.uri))
    const followsToAdd = actualFollowers.filter(x => !currentFollowers.map(b => b.uri).includes(x.uri))

    console.log(followsToAdd)
    console.log(followsToDelete)

    if (followsToAdd.length > 0 || followsToDelete.length > 0) {
        ctx.log(`- ${followsToAdd.length} to add, ${followsToDelete.length} to delete`)
    }

    if (followsToDelete.length > 0) {
      await ctx.db
      .deleteFrom('follows')
      .where('uri', 'in', followsToDelete.map(b => b.uri))
      .execute()
    }
